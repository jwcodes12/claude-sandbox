...
- Tonight's kind: [[kind]] — make [[kind_guidance]]
- Energy: [[energy_name]] (higher energy = more ambition is welcome)
- Recent builds (avoid repeating these): [[recent]]

Invent ONE concrete, self-contained idea of tonight's kind, buildable as a single
static index.html (no build step, no external network calls, no libraries fetched
at runtime). It must work offline and on a phone screen. Favor a strong, specific
hook over a generic demo. Delightful > complex.
