/**
 * mp-victory-stress.js — 5-Player Automated Victory Stress Test
 *
 * Runs up to 60 turns until someone wins.
 * Verifies synchronization, card effects, and state consistency.
 */
...
async function setupLobby(pages) {
    ...
    await Promise.all(pages.map(p => p.evaluate(() => {
        if (window.__game) {
            ...
            window.__game.state().players.forEach(pp => { 
                pp._isBot = false; 
                pp._difficulty = 'hard';
            });
        }
...
(async () => {
    console.log('\n╔══════════════════════════════════════════════════════════════╗');
    console.log('║  Lord Landlord — Automated Victory Stress Test               ║');
    console.log('║  WebRTC · Cross-Machine Sync · Hard Bot Strategy             ║');
    console.log('╚══════════════════════════════════════════════════════════════╝');
    ...
    try {
        ...
        while (turnCounter < MAX_TURNS) {
            ...
            if (state._gameOver) {
                const winner = state.players.find(p => {
                    ...
                    for (const k of Object.keys(p.properties)) {
                        const count = p.properties[k].length;
                        if (k === 'BROWN' || k === 'DARKBLUE') { if (count >= 2) sets++; }
                        else if (k === 'RAILROAD') { if (count >= 4) sets++; }
                        else if (k === 'UTILITY') { if (count >= 2) sets++; }
                        else { if (count >= 3) sets++; }
                    }
                    return sets >= 3;
                });
                ok(`GAME OVER! ${winner ? winner.name : 'Someone'} has claimed the crown in ${turnCounter} turns.`);
                break;
            }
            ...
            while (actionsInTurn < 10) {
                ...
                const cardInfo = await pages[currentTurnPlayer].evaluate((aid) => {
                    const st = window.__game.state();
                    const me = st.players[st.localPlayerId];
                    const c = me.hand.find(cc => cc.data.id === aid);
                ...
                await pages[currentTurnPlayer].waitForFunction((prev) => {
                    ...
                    throw new Error(`Action ${action.type} ${action.cardId} ("${cardInfo}") had no effect on state!`);
                });
        ...
        if (turnCounter >= MAX_TURNS) ok("Successfully completed MAX turns of high-intensity play!");
        
        console.log('\n  ── Card Type Coverage ──');
        console.log(`  Seen: ${Array.from(seenCards).sort().join(', ')}`);
